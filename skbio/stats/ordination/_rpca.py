...
def rpca_draft(
    ...
    n_components=3,
    min_sample_count=0,
    min_feature_count=0,  ### *********** These inputs can be made more general, I think
    ...
    if not isinstance(table, pd.DataFrame):
        raise ValueError(
            "Input table must be a pandas DataFrame."
        )  ### *********** Is this necessary? We should be using tabular format, right?

    if (table.values < 0).any():
        raise ValueError(
            "Input table contains negative values."
        )  ###  *************** RCLR will already raise this warning,
        ###                      so is this necessary?

    # Filter table
    table = _filter_table(
        table, min_sample_count, min_feature_count, min_feature_frequency
    )
    ...
    if table.shape[1] < n_components:
        raise ValueError(
            ...
            f"This are fewer than n_components ({n_components})."
        )
    ...
    rclr_table = rclr(table.values)

    # Apply OptSpace for matrix completion
    optspace = OptSpace(n_components=n_components, max_iterations=max_iterations)
    optspace.fit(rclr_table)

    ### *****************
    ### OptSpace already returns a singular value decomposition of the completed
    ### matrix, but this is not centered. So, we have to center and then compute the
    ### SVD *again*. It feels like there should be an optimization here, but I don't
    ### think the final SVD can be bypassed, since the completed matrix will not, in
    ### general, be centered. Maybe OptSpace can be altered?

    # Get the completed matrix
    completed = optspace.transform()

    ordination_results = pca(
        completed, method="svd", dimensions=n_components
    )  ### Find way to handle sample_ids ***********************

    ### Also, sparse options? **********************************

    ordination_results.short_method_name = "RPCA"
    ordination_results.long_method_name = "Robust Principal Component Analysis"
